...
- **БД**: Google BigQuery (за синтаксисом `DA.table`)
- **Мова**: SQL
- **Основні операції**: JOIN, GROUP BY, UNION ALL, Window Functions (SUM OVER)
...
1. Переконайтесь, що у вас є доступ до датасету `DA` в BigQuery
2. Виконайте запит з файлу `queries/revenue.sql`
3. Проаналізуйте результати, особливо колонку `goal_percent`:
   - **> 100%** - план перевиконується
   - **= 100%** - план виконується точно
   - **< 100%** - план не виконується
